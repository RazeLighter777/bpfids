...
impl MemoTable {
    fn var_for(&mut self, m: IdsMatch) -> String {
        if let Some(v) = self.vars.get(&m) { return v.clone(); }
        let name = format!("mm{}", self.next); self.next += 1;
        let code = format!("int {} = ({});", name, m.to_c());
        self.vars.insert(m, name.clone());
        self.inits.push((name.clone(), code));
        name
    }
    pub fn emit_decls(&self) -> String { self.inits.iter().map(|(_, c)| c.clone()).collect::<Vec<_>>().join("\n") }
}

impl IdsExpr {    // New variant that memoizes across calls via shared RefCell<MemoTable>
    pub fn to_c_with_memo(&self, memo: &RefCell<MemoTable>) -> String {
        match self {
            IdsExpr::Match(m) => memo.borrow_mut().var_for(*m),
            IdsExpr::And(l, r) => format!("({})&&({})", l.to_c_with_memo(memo), r.to_c_with_memo(memo)),
            IdsExpr::Or(l, r)  => format!("({})||({})", l.to_c_with_memo(memo), r.to_c_with_memo(memo)),
...
impl IdsRule {
    pub fn to_c_with_memo(&self, memo: &RefCell<MemoTable>) -> String {
        ...
        let action_c = match self.action {
            ...
            IdsAction::Alert => "bpf_printk(\"ALERT\\n\"); return XDP_DROP;".to_string(),
            IdsAction::Log => "bpf_printk(\"LOG\\n\"); return XDP_PASS;".to_string(),
        };
...
impl Config {
    pub fn to_c_memoized(&self) -> String {
        ...
        for r in &self.rules { rule_lines.push(r.to_c_with_memo(&memo)); }
...
impl IdsMatch {
    ...
    format!("(struct in6_addr){{ .s6_addr = {{ {} }} }}", bytes.join(", "))
    }
    fn ipv6_to_in6_addr_masked(ipv6: &std::net::Ipv6Addr, prefix: u8) -> String {
        ...
        if prefix < 128 {
            ...
            if rem_bits != 0 {
                ...
                o[full_bytes] &= mask;
                for b in &mut o[full_bytes + 1..] { *b = 0; }
            } else {
                for b in &mut o[full_bytes..] { *b = 0; }
            }
        }
        let bytes: Vec<String> = o.iter().map(|b| format!("0x{:02x}", b)).collect();
    format!("(struct in6_addr){{ .s6_addr = {{ {} }} }}", bytes.join(", "))
    ...
    pub fn to_c(&self) -> String {
        match self {
            ...
            IdsMatch::SourceNet(IpAddr::V6(ipv6), prefix) => {
                ...
                format!("({{ struct in6_addr tmp1 = apply_ipv6_netmask(&fctx.src_ip6, {}); struct in6_addr tmp2 = {}; __builtin_memcmp((const void*)&tmp1, (const void*)&tmp2, 16) == 0; }})", prefix, masked_addr)
            },
            IdsMatch::DestinationNet(IpAddr::V6(ipv6), prefix) => {
                let masked_addr = Self::ipv6_to_in6_addr_masked(ipv6, *prefix);
                format!("({{ struct in6_addr tmp1 = apply_ipv6_netmask(&fctx.dst_ip6, {}); struct in6_addr tmp2 = {}; __builtin_memcmp((const void*)&tmp1, (const void*)&tmp2, 16) == 0; }})", prefix, masked_addr)
            },
            ...
            _ => {
                ...
                let memcmp_arg1 = match self {
                    ...
                    IdsMatch::SourceNet(IpAddr::V4(_), prefix) => format!("(fctx.src_ip & {})", u32::MAX << (32 - prefix)),
                    IdsMatch::DestinationNet(IpAddr::V4(_), prefix) => format!("(fctx.dst_ip & {})", u32::MAX << (32 - prefix)),
                ...
                let memcmp_arg2 = match self {
                    IdsMatch::SourceHost(IpAddr::V4(ip)) => format!("0x{:08x}", u32::from(*ip)),
                    IdsMatch::DestinationHost(IpAddr::V4(ip)) => format!("0x{:08x}", u32::from(*ip)),
                    IdsMatch::SourceNet(IpAddr::V4(ip), l) => format!("0x{:08x}", u32::from(*ip) & (u32::MAX << (32 - l))),
                    IdsMatch::DestinationNet(IpAddr::V4(ip), l) => format!("0x{:08x}", u32::from(*ip) & (u32::MAX << (32 - l))),
                ...
                if use_eq_not_memcmp {
                    ...
                    format!("(__builtin_memcmp((const void*)&{}, (const void*)&{}, {}) == 0)", memcmp_arg1, memcmp_arg2, 16)
                }
            }
        }
    }
